...
public class MainApp extends Application {
    ...
    public void saveMaze() {
        ...
        if (result.isPresent() && !result.get().trim().isEmpty()) {
            ...
            // Sauvegarder le chemin de solution s'il existe
            if (currentSolver != null && !currentSolver.getCurrentPath().isEmpty()) {
                savedMaze.setSolutionPath(currentSolver.getCurrentPath());
            }
    ...
    public void loadSavedMaze(SavedMaze savedMaze) {
        ...
        // Restaurer le chemin de solution s'il existe
        if (savedMaze.hasSolutionPath() && startCell != null && goalCell != null) {
            List<Cell> path = savedMaze.getSolutionPath(grid);
            if (!path.isEmpty()) {
                // Créer un solveur pour stocker le chemin
                MazeSolver solver = SolverFactory.getSolverByName(selectedAlgorithm);
                solver.initializeStepByStep(grid, startCell, goalCell);
                
                // Définir le chemin et les cellules visitées
                solver.setCurrentPath(path);
                solver.setVisitedCells(path); // On considère que toutes les cellules du chemin ont été visitées
                
                // Stocker le solveur pour pouvoir l'utiliser plus tard
